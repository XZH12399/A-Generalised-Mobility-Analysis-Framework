...
1.  **构建运动旋量系统**: 将所有关节转换为螺旋 $\boldsymbol{\$}_i$。对于 R 副，$\boldsymbol{\$} = [\boldsymbol{\omega}; \boldsymbol{r} \times \boldsymbol{\omega}]$；对于 P 副，$\boldsymbol{\$} = [\boldsymbol{0}; \boldsymbol{v}]$。
2.  **雅可比矩阵构建**: 基于闭环约束方程 $\sum \dot{\theta}_i \boldsymbol{\$}_i = \boldsymbol{0}$ 构建全局雅可比矩阵 $\mathbf{K}$。
3.  **IDOF 检测 (Phase 2)**:
    * 计算二阶李括号项 $[\boldsymbol{\$}_i, \boldsymbol{\$}_j]$。
