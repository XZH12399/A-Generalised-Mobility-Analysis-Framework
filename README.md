  ...
  "nodes": [
    {
      ...
      "axis": [0, 0, 1],      // 关节运动方向；R 为转轴方向，P 为移动方向
      "pos": [0, 0, 0]        // 关节位置或几何参考点 [x, y, z]
    },
    ...
  ],
  "links": [
...
### 关键字段详解
1.  **`nodes`**: 定义所有运动副的位置和方向。`axis` 字段统一表示关节运动方向：对 R 副为转轴方向，对 P 副为移动方向；`pos` 字段表示关节位置或几何参考点。`mechanism_utils.py` 会自动将其转换为 Plücker 坐标（螺旋）。
2.  **`links`**: 定义机构的拓扑图。
